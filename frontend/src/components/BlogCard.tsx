...
export const BlogCard = ({
    ...
    <div className="border-b p-4 border-slate-200 pb-4 w-screen max-w-screen-md cursor-pointer">
        ...
        <div className="texy-md font-thin">
            {content.slice(0, 100) + "..."}
        </div>
...
export function Avatar({name, size = "small" }: {name:string,
    ...
    justify-center ${size === "small" ? "w-6 h-6" : "w-10 h-10"} overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600`}>
    <span className={`${size === "small" ? "text-sm" : "text-md"}text-xs font-extralight text-gray-600 dark:text-gray-300`}>{name[0]}</span>
</div>
}
